...
Ext.define('CACS.mvc.result.ViewResult', {
    ...
    execute: function (controller) {
        var action = this.action;//this.getCurrentAction();
        var frameId = this.frameId;
        var viewClass = controller ? controller.getView(this.view) : Ext.ClassManager.get(this.view);
        var viewInstance = this.getViewInstance(viewClass, frameId);
    ...
    getViewInstance: function (viewClass, frameId) {
        var viewInstance;
        var viewStore = this.getViewStore();
        //var viewClass = controller.getView(name);
        //查找框架Id
        if (Ext.isEmpty(frameId) || !Ext.isString(frameId)) {
            frameId = this.action;//this.getCurrentAction();
        }
        //判断框架是否存在
